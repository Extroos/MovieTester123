...
function formatVidlinkResponse(data: any, gatewayUrl = 'https://vidlink.pro'): any {
  if (!data || !data.stream) {
    return null;
  }
  ...
  if (stream.playlist) {
    const originalPlaylist = stream.playlist;
    const proxiedPlaylist = `${localServer}/local-proxy?url=${encodeURIComponent(originalPlaylist)}&referer=${encodeURIComponent(referer)}&origin=${encodeURIComponent(origin)}`;
    sources.push({
      url: proxiedPlaylist,
      quality: 'auto',
      isM3U8: stream.type === 'hls' || originalPlaylist.includes('.m3u8')
    });
  } else if (stream.qualities) {
    Object.entries(stream.qualities).forEach(([quality, qObj]: [string, any]) => {
      if (qObj && qObj.url) {
        const proxiedUrl = `${localServer}/local-proxy?url=${encodeURIComponent(qObj.url)}&referer=${encodeURIComponent(referer)}&origin=${encodeURIComponent(origin)}`;
        sources.push({
          url: proxiedUrl,
          quality: quality,
  ...
  const captionsList = stream.captions || [];
...
function getLocalServerUrl(): string {
  const stored = localStorage.getItem('cinemovie_consumet_url');
