  ...
  const handleDownloadOffline = async () => {
    ...
    try {
      ...
      if (iframeFallback || !playlistUrl || !playlistUrl.startsWith('http')) {
        ...
        let resolvedUrl = '';
        const serversToTry: ('vidlink-pro' | 'vidlink-me' | 'universal')[] = ['vidlink-pro', 'vidlink-me', 'universal'];
      ...
      if (isMp4) {
        if (Capacitor.isNativePlatform()) {
          setDownloadStatus('Downloading MP4 natively...');
          const res = await fetchWithCapacitor(targetUrl, 'arraybuffer');
          if (!res.ok) throw new Error("Failed to download video file.");
          const arrayBuffer = await res.arrayBuffer();
          const mergedArray = new Uint8Array(arrayBuffer);
          
          setDownloadStatus('Saving to offline storage...');
          const isTV = !!season || !!episode;
          const downloadId = isTV ? `tv_${item?.id}_${season}_${episode}` : `movie_${item?.id}`;
          const localPlayableUrl = await OfflineStorageService.save(downloadId, mergedArray);
          ...
          if (!existingItem) {
            existingItem = {
              id: downloadId,
              title: item?.title || (item as any)?.name || 'Video',
      ...
      for (let i = 0; i < totalSegments; i += batchSize) {
        ...
        const promises = batchUrls.map(async (url, idx) => {
          if (Capacitor.isNativePlatform()) {
            const res = await fetchWithCapacitor(url, 'arraybuffer');
            if (!res.ok) throw new Error(`Segment ${i + idx + 1} download failed.`);
            return res.arrayBuffer();
          } else {
            const segProxyUrl = buildProxyUrl(url);
            const response = await fetch(segProxyUrl);
            if (!response.ok) throw new Error(`Segment ${i + idx + 1} download failed.`);
