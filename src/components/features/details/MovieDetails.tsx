    ...
    try {
      ...
      if (result && result.streamUrl) {
        ...
        if (isDirectVideo && Capacitor.isNativePlatform()) {
          ...
          // Start simulated progress interval to update both local state and Downloads page in real-time
          let progressVal = 0;
          const progressInterval = setInterval(() => {
            if (progressVal < 99) {
              const increment = progressVal < 50 ? 3 : progressVal < 80 ? 1.5 : 0.5;
              progressVal = Math.min(99, progressVal + increment);
              setDownloadProgress(Math.floor(progressVal));
          ...
          const promises = batchUrls.map(async (url, idx) => {
            if (Capacitor.isNativePlatform()) {
              ...
              if (!response || !response.ok) {
                console.warn(`[MovieDetails] Native Segment ${i + idx + 1} download failed after 5 attempts. Skipping to keep download alive.`);
            ...
            if (!response || !response.ok) {
              console.warn(`[MovieDetails] Segment ${i + idx + 1} download failed after 5 attempts. Skipping to keep download alive.`);
