...
app.get('/meta/tmdb/watch/:tmdbId', async (req, res) => {
  ...
  // universal / auto failover: try VidSrc fallback first, then Vidify
  if (server === 'auto') {
    try {
      const pythonUrl = type === 'tv'
        ? `http://localhost:8000/fallback/tv/${tmdbId}/${season}/${episode}`
        : `http://localhost:8000/fallback/movie/${tmdbId}`;
      console.log(`[Server] Universal: Trying Python VidSrc fallback: ${pythonUrl}`);
      const res2 = await fetch(pythonUrl);
      if (res2.ok) {
        const result = await res2.json();
        return res.json(rewriteLocalhostUrls(result, req));
  ...
  // vidsrc or auto failover: use the Python fallback scraper (extracts direct .m3u8 streams)
  if (server === 'vidsrc' || server === 'auto') {
    try {
      const pythonUrl = type === 'tv'
        ? `http://localhost:8000/fallback/tv/${tmdbId}/${season}/${episode}`
        : `http://localhost:8000/fallback/movie/${tmdbId}`;
