...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            if (path.startsWith('/vidsrc-cc/')) return handleVidSrcCc(path, respond);
...
async function handleVidSrcCc(path, respond) {
    try {
        const targetPath = path.replace('/vidsrc-cc', '');
        const res = await fetch(`https://vidsrc.cc${targetPath}`, { signal: AbortSignal.timeout(10000) });
